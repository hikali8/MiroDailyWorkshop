...
int RGBAToNV21(const uint8_t* src_rgba,
    ...
    int y;
    int halfwidth = (width + 1) >> 1;
    void (*RGBAToUVRow)(const uint8_t* src_argb0, int src_stride_argb,
                        uint8_t* dst_u, uint8_t* dst_v, int width) =
    RGBAToUVRow_C;
    void (*RGBAToYRow)(const uint8_t* src_argb, uint8_t* dst_y, int width) =
    RGBAToYRow_C;
    void (*MergeUVRow_)(const uint8_t* src_u, const uint8_t* src_v,
                        uint8_t* dst_vu, int width) = MergeUVRow_C;
    ...
    if (height < 0) {
        height = -height;
        src_rgba += (height - 1) * src_stride_rgba;
...
#endif
#if defined(HAS_RGBATOYROW_AVX2)
    if (TestCpuFlag(kCpuHasAVX2)) {
...
#endif
#if defined(HAS_RGBATOUVROW_AVX2)
    if (TestCpuFlag(kCpuHasAVX2)) {
...
#endif
#if defined(HAS_RGBATOYROW_LASX) && defined(HAS_RGBATOUVROW_LASX)
    if (TestCpuFlag(kCpuHasLASX)) {
    RGBAToYRow = RGBAToYRow_Any_LASX;
    RGBAToUVRow = RGBAToUVRow_Any_LASX;
    if (IS_ALIGNED(width, 32)) {
      RGBAToYRow = RGBAToYRow_LASX;
      RGBAToUVRow = RGBAToUVRow_LASX;
    }
  }
#endif
#if defined(HAS_RGBATOYROW_RVV)
    if (TestCpuFlag(kCpuHasRVV)) {
    RGBAToYRow = ARGBToYRow_RVV;
  }
#endif
#if defined(HAS_MERGEUVROW_SSE2)
    if (TestCpuFlag(kCpuHasSSE2)) {
